...
void drawText(int x, int y, string mess) {
	RECT rect;
	const char* messChange = (char*)mess.c_str();
	LPCWSTR message = convertCharArrayToLPCWSTR(messChange);
...
void outtextxy(int x, int y, int text)
{
	gotoxy(x/8,y/12);
	std::cout << text;
}
